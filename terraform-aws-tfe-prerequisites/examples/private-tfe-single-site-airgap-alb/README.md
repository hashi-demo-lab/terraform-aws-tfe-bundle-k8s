# Private TFE (Replicated) Deployment with an ALB Example
...
This module showcases an example of utilizing the root module of this repository to build out the following high level components for a Private TFE deployment inside of AWS in a **specific region**:  
...
* Certificate for the ALB to use in ACM
...
The Application Load Balancer will use a certificate provided by the end user.  This will need to be stored in ACM for use with this module.  

1. Pre-create the private zone and associate it with your default VPC in your account.
2. You also need to upload the certificates inside of ACM for the ALB to use (if you are going to use the ALB).
3. Once you upload the certificate you need to copy the ARN and place it in the `terraform.auto.tfvars`
4. Once the run is done then associate the private zone with the VPC built in this example. You can also do this via Terraform using this resource here <https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/route53_zone_association>
---
...
| <a name="input_lb_certificate_arn"></a> [lb\_certificate\_arn](#input\_lb\_certificate\_arn) | ARN of the existing certificate to use with the load balacner | `string` | `null` | no |
