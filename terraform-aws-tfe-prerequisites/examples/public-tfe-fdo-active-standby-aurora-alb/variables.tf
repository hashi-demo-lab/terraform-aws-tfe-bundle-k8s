...
variable "primary_route53_failover_record" {
  type = object({
    create              = optional(bool, true)
...
variable "secondary_route53_failover_record" {
  type = object({
    create              = optional(bool, true)
    set_id              = optional(string, "fso1")
    lb_failover_primary = optional(bool, true)
...
variable "tfe_hostname" {
  type        = string
  description = "Fully qualified domain name for the TFE instances to use. This is utilized as a string input so that the destroy operations will complete cleanly if you are tearing down a multi-site deployment."
}
...
variable "product" {
  type        = string
  description = "Name of the HashiCorp product that will consume this service (tfe, tfefdo, vault, consul, nomad, and boundary)"
  validation {
    condition     = contains(["tfe", "tfefdo", "vault", "consul", "nomad", "boundary"], var.product)
    error_message = "`var.product` must be \"tfe\", \"tfefdo\", \"vault\", \"nomad\", \"boundary\", or \"consul\"."
  }
}
